    ...
    <section class="space-y-8">
      ...
      <div class="rounded-2xl border border-dls-border bg-dls-surface px-5 py-5 shadow-[0_8px_26px_rgba(17,24,39,0.05)]">
        <div class="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div class="min-w-0 space-y-1">
            ...
            <p class="text-sm text-dls-secondary">{translate("skills.worker_profile_desc")}</p>
          </div>
          <button
            type="button"
          ...
          >
            <Sparkles size={14} />
            {translate("skills.create_skill_in_chat")}
          </button>
        </div>

        <div class="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
          <div class="rounded-lg border border-dls-border bg-dls-hover px-3 py-2.5">
            <div class="text-[11px] text-dls-secondary">{translate("skills.card_installed")}</div>
            <div class="mt-1 text-base font-semibold text-dls-text">{props.skills.length}</div>
          </div>
          <div class="rounded-lg border border-dls-border bg-dls-hover px-3 py-2.5">
            <div class="text-[11px] text-dls-secondary">{translate("skills.card_hub_available")}</div>
            <div class="mt-1 text-base font-semibold text-dls-text">{availableHubSkills().length}</div>
          </div>
          <div class="rounded-lg border border-dls-border bg-dls-hover px-3 py-2.5">
            <div class="text-[11px] text-dls-secondary">{translate("skills.card_skill_creator")}</div>
            <div class="mt-1 text-base font-semibold text-dls-text">
              {skillCreatorInstalled() ? translate("skills.card_installed") : translate("skills.card_not_installed")}
            </div>
          </div>
          <div class="rounded-lg border border-dls-border bg-dls-hover px-3 py-2.5">
            <div class="text-[11px] text-dls-secondary">{translate("skills.card_mode")}</div>
            <div class="mt-1 text-base font-semibold text-dls-text">
              {props.canUseDesktopTools ? translate("skills.card_mode_local") : translate("skills.card_mode_server")}
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-wrap items-center gap-3 border-b border-dls-border pb-4">
        ...
        <div class="relative">
          ...
          <input
            ...
            placeholder={translate("skills.search_hub_placeholder")}
        ...
        <button
          ...
          title={translate("skills.install_from_link_title")}
        >
          <Link2 size={14} />
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between gap-3">
          <h3 class="text-[11px] font-bold text-dls-secondary uppercase tracking-widest">{translate("skills.install_section_title")}</h3>
          <button
            type="button"
        ...
        <Show
          ...
          fallback={
            <div class="rounded-xl border border-dls-border bg-dls-surface px-5 py-6 text-sm text-dls-secondary">
              {translate("skills.no_hub_available")}
            </div>
          }
        >
          <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              {(skill) => (
                <div class="bg-dls-surface border border-dls-border rounded-xl p-4 flex items-start justify-between gap-4 group hover:border-dls-border hover:bg-dls-hover transition-all text-left">
                  ...
                  <button
                    ...
                    title={translate("skills.install_skill_title").replace("{name}", skill.name)}
                  >
                    <Show
      ...
      <Show when={installLinkOpen()}>
        <div class="fixed inset-0 z-50 bg-black/20 backdrop-blur-sm flex items-center justify-center p-4">
          <div class="bg-dls-surface border border-dls-border w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden">
            <div class="p-6 space-y-4">
              <div>
                <h3 class="text-lg font-semibold text-dls-text">{translate("skills.install_from_link_modal_title")}</h3>
                <p class="text-sm text-dls-secondary mt-1">{translate("skills.install_from_link_modal_desc")}</p>
              </div>

              <div class="space-y-2">
                <div class="text-xs font-semibold uppercase tracking-widest text-dls-secondary">{translate("skills.install_link_label")}</div>
                <input
                  type="url"
                ...
                {(bundle) => {
                  ...
                  return (
                    <div class="rounded-xl border border-dls-border bg-dls-hover p-4 space-y-2">
                      ...
                      <Show when={conflict}>
                        <div class="text-xs text-amber-11">{translate("skills.skill_conflict")}</div>
                      </Show>
                    </div>
                  );
                }}
