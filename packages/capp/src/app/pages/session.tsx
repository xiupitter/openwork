...
export default function SessionView(props: SessionViewProps) {
  ...
  const handleSoulQuickstart = async () => {
    ...
    const text = SOUL_SETUP_TEMPLATE.body || t("session.hero_soul_title");
    handleSendPrompt({
      mode: "prompt",
      text,
      resolvedText: text,
          ...
          <div class="flex-1 min-w-0 relative overflow-hidden bg-gray-1">
            ...
            <Show when={showWorkspaceSetupEmptyState()}>
              <div class="mx-auto max-w-xl rounded-3xl border border-gray-6 bg-gray-2/60 p-8 text-center shadow-sm">
                ...
                <h3 class="text-2xl font-semibold text-gray-12">{t("worker.setup_first_title")}</h3>
                <p class="mt-2 text-sm text-gray-10">
                  {t("worker.setup_first_description")}
                </p>
            ...
            <Show when={props.messages.length === 0 && !showWorkspaceSetupEmptyState()}>
              ...
              <div class="grid gap-3 sm:grid-cols-2 max-w-2xl mx-auto text-left">
                ...
                >
                  <div class="text-sm font-semibold text-dls-text">{t("session.hero_browser_title")}</div>
                  <div class="mt-1 text-xs text-dls-secondary leading-relaxed">
                    {t("session.hero_browser_description")}
                  </div>
                </button>
                <button
                  type="button"
                ...
                >
                  <div class="text-sm font-semibold text-dls-text">{t("session.hero_soul_title")}</div>
                  <div class="mt-1 text-xs text-dls-secondary leading-relaxed">
                    {t("session.hero_soul_description")}
                  </div>
                </button>
              </div>
            </div>
